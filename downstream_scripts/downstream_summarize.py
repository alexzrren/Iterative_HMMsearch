...
Polyomaviridae'''.splitlines()
    ...
    def votevf(input_tuple):
        ...
        bitscore_voting = votedf.groupby('nr_svfamily')['nr_bitscore'].sum() #对分类结果进行投票
    ...
    blastnr_full['nr_vertebrate'] = blastnr_full['nr_vfamily_add75'].isin(nr_vertebrate_vflist)
...
if __name__=='__main__':
    ...
    rna_domcov, rna_bestorflist = filt_motifcov('output_rna_v1')
    ...
    summary_full2 = pd.merge(summary_concise1, rdrpscan_full, on='qseqid', how='left')
    ...
    #orfpos = pd.read_table('/data/user/renzirui/Projects/GIZ_GameMarket/orf.position.tsv', header=None, names='qorfid orfstart orfend'.split())
    summary_clean_vert = summary_clean[(summary_clean.nr_vertebrate) & (summary_clean.vhdb_vertebrate.fillna(True))]
    #summary_clean_vert = pd.merge(summary_clean_vert, orfpos, on='qorfid', how='left')
    summary_clean_vert_specrmdup = summary_clean_vert.sort_values('qlen', ascending=False).drop_duplicates('Cluster_AAI90')

    with pd.ExcelWriter('WIV_ChinaBat.xlsx') as writer:
        summary_clean_vert.to_excel(writer, sheet_name='VertVirus', index=False)
        summary_clean_vert_specrmdup.to_excel(writer, sheet_name='VertVirus (rmdup)', index=False)
        summary_clean_vert[['Cluster_AAI90', 'qseqid']].to_excel(writer, sheet_name='ClusterInfo', index=False)
        summary_concise5.to_excel(writer, sheet_name='VirusInfo (Concise)', index=False)
        summary_full5.to_excel(writer, sheet_name='VirusInfo (Full)', index=False)
        #blastnr_fp_full.to_excel(writer, sheet_name='NR_FP', index=None)    
